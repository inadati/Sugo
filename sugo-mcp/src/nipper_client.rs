...
pub async fn inject(base: &str, token_path: &str, project_path: &str, text: &str) -> NipperOutcome {
    post(
        ...
        "/inject",
        token_path,
        json!({ "project_path": project_path, "text": text }),
    )
    .await
}
...
pub async fn detach(
    ...
    post(
        ...
        "/detach",
        token_path,
        json!({ "project_path": project_path, "run_id": run_id }),
    )
    .await
}
...
mod tests {
    ...
    async fn spawn_mock(status: StatusCode) -> String {
        let app = Router::new().route(
            ...
            post(move |Json(_): Json<Value>| async move {
                (status, Json(json!({"status":"buffered"})))
            }),
        );
    ...
    async fn inject_200_is_ok() {
        ...
        assert_eq!(
            inject(&base, &token_path, "/p", "hi").await,
            NipperOutcome::Ok
        );
    }
    ...
    async fn missing_token_file_is_token_unavailable_without_network_call() {
        ...
        let missing_path =
            std::env::temp_dir().join(format!("sugo-test-missing-{}", uuid::Uuid::new_v4()));
        assert_eq!(
            inject(
                "http://127.0.0.1:1",
    ...
    async fn detach_sends_run_id_in_body() {
        ...
        assert_eq!(
            detach(&base, &token_path, "/p", "run-xyz").await,
            NipperOutcome::Ok
        );
        assert_eq!(
            captured
    ...
    async fn read_token_missing_file_is_not_found() {
        ...
        assert_eq!(
            read_token(path.to_str().unwrap()).await,
